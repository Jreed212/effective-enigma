...
(function(){
 ...
 lastAccessory=function(name){const names=accessoryAliases[name]||[name];for(const l of [...p().logs].reverse()){const it=(l.items||[]).find(x=>names.includes(x.name));if(!it)continue;const sets=it.sets||[],last=sets.filter(s=>s.touched||s.done).pop();if(!last)continue;let wt=+last.weight||0;const promote=sets.length&&sets.every(s=>(+s.reps||0)>=12)&&rirNum(it.rir)>=2;if(promote)wt+=5;return{weight:wt,note:(it.name!==name?'Carry forward from '+it.name:promote?'Earned +5 lb':'Carry forward')}}return baseLastAccessory(name)};
